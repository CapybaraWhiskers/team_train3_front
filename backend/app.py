...
@app.route("/msal-login", methods=["POST"])
def msal_login():
    data = request.get_json() or {}
    email = data.get("email")
    name = data.get("name")
    ...
    with get_db() as conn:
        cur = conn.execute("SELECT id, name, role FROM users WHERE email=?", (email,))
        row = cur.fetchone()
        if row:
            # 既存ユーザー: セッションセット
            session["user_id"] = row["id"]
            ...
            session["role"] = role
            session["email"] = email
    return jsonify({"status": "logged_in", "name": session["name"], "role": session["role"]})
